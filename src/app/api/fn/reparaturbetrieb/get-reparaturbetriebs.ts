...
export interface GetReparaturbetriebs$Params {
...
export function getReparaturbetriebs(http: HttpClient, rootUrl: string, params?: GetReparaturbetriebs$Params, context?: HttpContext): Observable<StrictHttpResponse<ReparaturbetriebListResponse>> {
  const rb = new RequestBuilder(rootUrl, getReparaturbetriebs.PATH, 'get');
  if (params) {
  ...
  return http.request(
    ...
    map((r: HttpResponse<any>) => {
      return r as StrictHttpResponse<ReparaturbetriebListResponse>;
    })
  );
}

getReparaturbetriebs.PATH = '/reparaturbetriebs';
