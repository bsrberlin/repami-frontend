...
export interface GetProductCategories$Params {
...
export function getProductCategories(http: HttpClient, rootUrl: string, params?: GetProductCategories$Params, context?: HttpContext): Observable<StrictHttpResponse<ProductCategoryListResponse>> {
  const rb = new RequestBuilder(rootUrl, getProductCategories.PATH, 'get');
  if (params) {
  ...
  return http.request(
    ...
    map((r: HttpResponse<any>) => {
      return r as StrictHttpResponse<ProductCategoryListResponse>;
    })
  );
}

getProductCategories.PATH = '/product-categories';
