...
export interface GetReparaturcafes$Params {
...
export function getReparaturcafes(http: HttpClient, rootUrl: string, params?: GetReparaturcafes$Params, context?: HttpContext): Observable<StrictHttpResponse<ReparaturcafeListResponse>> {
  const rb = new RequestBuilder(rootUrl, getReparaturcafes.PATH, 'get');
  if (params) {
  ...
  return http.request(
    ...
    map((r: HttpResponse<any>) => {
      return r as StrictHttpResponse<ReparaturcafeListResponse>;
    })
  );
}

getReparaturcafes.PATH = '/reparaturcafes';
