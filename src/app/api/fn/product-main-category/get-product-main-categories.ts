...
export interface GetProductMainCategories$Params {
...
export function getProductMainCategories(http: HttpClient, rootUrl: string, params?: GetProductMainCategories$Params, context?: HttpContext): Observable<StrictHttpResponse<ProductMainCategoryListResponse>> {
  const rb = new RequestBuilder(rootUrl, getProductMainCategories.PATH, 'get');
  if (params) {
  ...
  return http.request(
    ...
    map((r: HttpResponse<any>) => {
      return r as StrictHttpResponse<ProductMainCategoryListResponse>;
    })
  );
}

getProductMainCategories.PATH = '/product-main-categories';
